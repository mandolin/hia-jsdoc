...
for (const packageRoot of packageRoots) {
  ...
  for (const file of pack.files || []) {
    ...
    if (filePath.includes("/node_modules/") || filePath.startsWith("node_modules/") || filePath.endsWith(".tgz")) {
      console.error(`Unsafe file in HIA JSDoc pack dry-run: ${file.path}`);
      failed = true;
    }
  }
  if (packageJson.name === "@hia-doc/jsdoc-runner" || packageJson.name === "@hia-doc/jsdoc-producer") {
    for (const requiredPath of ["README.md", "package.json", "src/index.mjs"]) {
  ...
  if (packageJson.name === "@hia-doc/jsdoc-runner" && !packedPaths.has("src/cli.mjs")) {
    console.error("Missing src/cli.mjs in @hia-doc/jsdoc-runner pack dry-run.");
    failed = true;
  }
}
...
console.log(`HIA JSDoc pack check passed: ${packageRoots.length} workspace packages.`);
